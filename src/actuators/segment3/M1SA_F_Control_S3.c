...
void M1SA_F_Control_S3_step(void)
{
  ...
  for (i = 0; i < 6; i++) {
    ...
    i_1 = 0;
    for (i_0 = 0; i_0 < 6; i_0++) {
      rtb_LC2CG[i] += M1SA_F_Control_S3_ConstP.LC2CG_Gain[i_1 + i] *
  ...
  for (i_0 = 0; i_0 < 335; i_0++) {
    ...
    i = 0;
    for (i_1 = 0; i_1 < 6; i_1++) {
      rtb_SADynamicsS1[i_0] += M1SA_F_Control_S3_ConstP.ForcebalanceS1_Gain[i +
        i_0] * tmp[i_1];
      i += 335;
    }
     ...
     *  Sum: '<S1>/Add'
     */
    denAccum = (rtb_SADynamicsS1[i_0] + M1SA_F_Control_S3_U.SA_offsetF_cmd[i_0])
      - -0.53348809109110329 * M1SA_F_Control_S3_DW.SADynamicsS1_states[i_0];
    SADynamicsS1_tmp[i_0] = denAccum;
    denAccum *= 0.25752323685913081;
    rtb_SADynamicsS1[i_0] = 0.20898867204976596 *
      M1SA_F_Control_S3_DW.SADynamicsS1_states[i_0] + denAccum;

    /* Outport: '<Root>/M1_ACT_F' incorporates:
     *  Gain: '<S1>/Force balance S1'
     */
    M1SA_F_Control_S3_Y.M1_ACT_F[i_0] = rtb_SADynamicsS1[i_0];
  }
  ...
  memcpy(&M1SA_F_Control_S3_DW.SADynamicsS1_states[0], &SADynamicsS1_tmp[0],
         335U * sizeof(real_T));
}
