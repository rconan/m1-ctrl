...
void M1SA_F_Control_S4_step(void)
{
  ...
  for (k = 0; k < 27; k++) {
    memOffset_tmp = k << 1;
    denAccum_tmp = M1SA_F_Control_S4_DW.SADynamics_states[memOffset_tmp + 1];
    denAccum = (M1SA_F_Control_S4_U.BM_cmd[k] - -0.77932227475069393 *
                M1SA_F_Control_S4_DW.SADynamics_states[memOffset_tmp]) -
  ...
  for (k = 0; k < 6; k++) {
    ...
    memOffset_tmp = 0;
    for (i = 0; i < 6; i++) {
      rtb_LC2CG[k] += M1SA_F_Control_S4_ConstP.LC2CG_Gain[memOffset_tmp + k] *
        M1SA_F_Control_S4_U.HP_LC[i];
      memOffset_tmp += 6;
    }
  }
  ...
  for (k = 0; k < 6; k++) {
    memOffset_tmp = k << 1;
    denAccum_tmp = M1SA_F_Control_S4_DW.SADynamicsS1_states[memOffset_tmp + 1];
    denAccum = (rtb_TmpSignalConversionAtSADynamicsS1Inport1[k] -
                -0.77932227475069393 *
                M1SA_F_Control_S4_DW.SADynamicsS1_states[memOffset_tmp]) -
      denAccum_tmp * 0.15183580198064905;
  ...
  for (k = 0; k < 335; k++) {
    ...
    for (memOffset_tmp = 0; memOffset_tmp < 6; memOffset_tmp++) {
      tmp[k] += M1SA_F_Control_S4_ConstP.ForcebalanceS1_Gain[335 * memOffset_tmp
    ...
    for (memOffset_tmp = 0; memOffset_tmp < 27; memOffset_tmp++) {
      tmp_0[k] += M1SA_F_Control_S4_ConstP.BM2Forces_Gain[335 * memOffset_tmp +
     ...
     *  Sum: '<S1>/Add'
     */
    M1SA_F_Control_S4_Y.M1_ACT_F[k] = tmp[k] + tmp_0[k];
  }

  /* Update for DiscreteTransferFcn: '<S1>/SA Dynamics' */
  for (k = 0; k < 27; k++) {
    memOffset_tmp = k << 1;
    M1SA_F_Control_S4_DW.SADynamics_states[memOffset_tmp - -1] =
      M1SA_F_Control_S4_DW.SADynamics_states[memOffset_tmp];
    M1SA_F_Control_S4_DW.SADynamics_states[memOffset_tmp] = SADynamics_tmp[k];
  }
  ...
  for (k = 0; k < 6; k++) {
    memOffset_tmp = k << 1;
    M1SA_F_Control_S4_DW.SADynamicsS1_states[memOffset_tmp - -1] =
      M1SA_F_Control_S4_DW.SADynamicsS1_states[memOffset_tmp];
    M1SA_F_Control_S4_DW.SADynamicsS1_states[memOffset_tmp] = SADynamicsS1_tmp[k];
  }
