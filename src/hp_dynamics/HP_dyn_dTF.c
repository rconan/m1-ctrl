...
DW_HP_dyn_dTF_T HP_dyn_dTF_DW;

/* External inputs (root inport signals with default storage) */
ExtU_HP_dyn_dTF_T HP_dyn_dTF_U;

/* External outputs (root outports fed by signals with default storage) */
ExtY_HP_dyn_dTF_T HP_dyn_dTF_Y;

/* Real-time model */
static RT_MODEL_HP_dyn_dTF_T HP_dyn_dTF_M_;
RT_MODEL_HP_dyn_dTF_T *const HP_dyn_dTF_M = &HP_dyn_dTF_M_;

/* Model step function */
void HP_dyn_dTF_step(void)
{
  ...
  for (k = 0; k < 6; k++) {
    ...
    denAccum = (((HP_dyn_dTF_U.In1[k] - -3.9324030529437657 *
                 ...
                 5.7980159366164923) - HP_dyn_dTF_DW.HP_dyn_dTF_states[memOffset
                + 2] * -3.7988189837355097) -
...
void HP_dyn_dTF_initialize(void)
{
  /* Registration code */

  /* initialize error status */
  rtmSetErrorStatus(HP_dyn_dTF_M, (NULL));
  ...
  /* external inputs */
  (void)memset(&HP_dyn_dTF_U, 0, sizeof(ExtU_HP_dyn_dTF_T));
...
void HP_dyn_dTF_terminate(void)
{
